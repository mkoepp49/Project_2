...
<html lang="en">
  <head>
	...
	<!-- Link to the relevant css file from the folder -->
    ...
    <div id="login">
      <div id="form">
        <div class="top">
			...
			<?php
			// all the variables being inputted from the form action of AdminSerchApp.php
				$date = $_POST["date"];
				$times = $_POST["time"];
				...
				// If there is no date, leave to all possible dates
				if($date == ''){ echo "Date: All"; }
				// otherwise set the date variable to the date user specified
				else{ 
					echo "Date: ",$date;
					$date = date('Y-m-d', strtotime($date));
				}
				echo "<br>";
				// if the time is empty, leave to all possible times
				if(empty($times)){ echo "Time: All"; }
				// otherwise set to the time user specified
				else{
					$i = 0;
					echo "Time: ";
					foreach($times as $t){
				...
				echo "<br>";
				// if advisor field was left open, set to all advisor appts (group or ind)
				if($advisor == ''){ echo "Advisor: All appointments"; }
				// segregate to only individual
				elseif($advisor == 'I'){ echo "Advisor: All individual appointments"; }
				// segregate to only group
				...
				<br><br><label>
				<?php
				// make selections from DB from Filters of search
				if(empty($times)){
					if($advisor == 'I'){
						if($filter == 1){
					...
					// keep picking from the row if not the first case
					if($row){
				...
				// keep segregating down, if Advisor set to individual
				else{
					if($advisor == 'I'){
						foreach($times as $t){
							if($filter == 1){
