<?php
// this file searches for student in the appoinment slot specified
session_start();
$debug = false;
    ...
    <div id="login">
      <div id="form">
        <div class="top">
			...
			<?php
				// gets the input chosen by admin in previous page
				$date = $_POST["date"];
				$times = $_POST["time"];
				...
				// if no dates are selected, then it means all dates
				// else just the specified date
				...
				echo "<br>";
				// checks for filter placed on which advisor
				...
				elseif($advisor == '0'){ echo "Advisor: All group appointments"; }
					...
					// if not then get row from advisor chosen
				...
				echo "<br>";
				// if no student id, then all students, else use student id + last name from DB
				...
				echo "<br>";
				// filter if statments for types of appointments
				...
				<br><br><label>
				<?php
				// get appointment for each filter type, with no time requirment
				if(empty($times)){
					if($advisor == 'I'){
						// here closed individual appointments
						if($filter == 1){
							...
							// individual appointmetns with all open/closed
							$sql = "select * from Proj2Appointments where `Time` like '%$date%' and 
								`AdvisorID` != 0 and 
								`EnrolledID` like '%$studID%' and 
						...
						// here closed group appointments
						if($filter == 1){
							$sql = "select * from Proj2Appointments where `Time` like '%$date%' and 
							...
							// group appointmetns with all open/closed
							$sql = "select * from Proj2Appointments where `Time` like '%$date%' and 
								`AdvisorID` like '%$advisor%' and 
								`EnrolledID` like '%$studID%' and 
								`EnrolledNum` like '%$filter%' order by `Time` ASC";
					...
					// get names etc from DB and push into array
					$rs = $COMMON->executeQuery($sql, $_SERVER["SCRIPT_NAME"]);
					$row = mysql_fetch_row($rs);
					...
					// do same as the code above, except this time theres a time requirement
					if($advisor == 'I'){
						foreach($times as $t){
							if($filter == 1){
				...
				// if no results are found, results array is emtpy
				if(empty($results)){
					echo "No results found.<br><br>";
				}
				else{
					// else print each item in array!
