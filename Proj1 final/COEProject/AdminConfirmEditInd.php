<?php
// This file will show the what appointment was removed
session_start();
?>
      ...
      <div id="form">
        <div class="top">
          ...
          <?php
            ...
            // get values and store to variables -----

            // get row from DB using advisor name
            $sql = "SELECT `id` FROM `Proj2Advisors` WHERE `FirstName` = '$row[1]' AND `LastName` = '$row[2]'";
            $rs = $COMMON->executeQuery($sql, "Advising Appointments");
            $rod = mysql_fetch_row($rs);
            $adv = $rod[0];

            // check if student enrolled, meaning theres a student ID
            if($row[4]){
            ...
            // print out the variables made, time and date, advisor, majors, enrolled
            ...
            // then set row in data base to "C" since appointment cancelled
            echo("Enrolled: ");
            if($row[4]){
              echo("$std</b>");
		...
		"http://coeadvising.umbc.edu  -> COEIT Advising Scheduling \r\n Reminder, this is only accessible on campus."; 
              // send email with message above to student's email 
