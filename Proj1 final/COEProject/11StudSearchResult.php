...
// This file displays all the avilable appointments for particular advisor
$debug = false;
    ...
    <div id="login">
      <div id="form">
	    ...
	    <div class="field">
			...
			<?php
				// the date chosen to see appointments
				$date = $_POST["date"];
				$times = $_POST["time"];
				$advisor = $_POST["advisor"];
				$results = array();
				
				// check if no date is chosen == all open
				if($date == ''){ echo "Date: All"; }
				// get date from database as a strtotime english format
				else{ 
					echo "Date: ",$date;
				...
				echo "<br>";
				// group/all/individual advisor result
				...
				<br><br><label>
				<?php
				// if no specific time is chosen
				if(empty($times)){
					// individual advisor, get appointments
					if($advisor == 'I'){
					...
					if($row){
						//  make the array of free appointment times
				...
				// specific time chosen for appointment
				else{
					if($advisor == 'I'){
						foreach($times as $t){
				...
				// if no open appointments were found
				if(empty($results)){
					echo "No results found.<br><br>";
				}
				else{
					// create table to print array of open appointments
