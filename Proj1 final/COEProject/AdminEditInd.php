<?php
// this file will allow admin to edit individual type appointments
session_start();
?>
      ...
      <div id="form">
        <div class="top">
          ...
          <?php
            ...
            //  gets first appointment, and starts from when its not 0
            //  meaning no a group appointment
            //  also stars from the current date, that way only appointments from current date show up
            $sql = "SELECT * FROM `Proj2Appointments` WHERE `AdvisorID` != '0' and `Time` > '".date('Y-m-d H:i:s')."' ORDER BY `Time`";
            $rs = $COMMON->executeQuery($sql, "Advising Appointments");
            $row = mysql_fetch_array($rs, MYSQL_NUM); 
			//first item in row
            // sends the input chosen by admin to the php file names below
            if($row){
              echo("<form action=\"AdminConfirmEditInd.php\" method=\"post\" name=\"Confirm\">");
              
  // creates a table to display appointments
	echo("<table border='1px'>\n<tr>");
	echo("<tr><td width='320px'>Time</td><td>Majors</td><td>Enrolled</td></tr>\n");
              // gets appointment by admin name from DB
              ...
              // if someone is enrolled, gets their student id
              if($row[4]){
			  ...
			  echo("</tr>\n");
			         ...
			         //get rest of items in row using a while loop
              ...
              // buttons for admin to choose
              // delete appointment or return to home to AdminUi
