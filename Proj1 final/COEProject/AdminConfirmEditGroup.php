...
<html lang="en">
  ...
  <body>
    <div id="login">
      <div id="form">
        ...
        <?php
          ...
          // Checks if admin called to delete
          // if so, remove appointment
          if($delete == true){
            echo("<h1>Removed Appointment</h1><br>");

            // select the row of the appoint using the row #
	...
	echo($stds[0]);
	    ...
	    $stds = split(" ", $stds);
            // check var_dump for any emails?
            if($debug) { var_dump("\n<BR>EMAILS ARE: $stds \n<BR>"); }
		// foreach($stds as $element) { echo("->".$element."\n"); }

            // if the row is found from mysql_fetch_row, and to be deleted
            if($stds)
	    {

              // update each appointment with "C" in the status, meaning appointment cancelled
		...
		"http://coeadvising.umbc.edu  -> COEIT Advising Scheduling \r\n Reminder, this is only accessible on campus."; 
                // send message above to email!
                mail($eml, "Your COE Advising Appointment Has Been Deleted", $message);
              }
            }
            // delete all info form the row in appointments
            ...
            // print out information to admin
            // here if only one major prints out major
            // else avilable to all 
            ...
            // checks if students have been notified
            if($stds[0]){
              echo "<p style='color:red'>Students have been notified of the cancellation.</p>";
            }
          }
          // if  admin is editing appointment, prints out old, then updated appointment
          else{
            echo("<h1>Changed Appointment</h1><br>");
			echo("<h2>Previous Appointment:</h2>");
            echo("Time: ". date('l, F d, Y g:i A', strtotime($row[0])). "<br>");
            echo("Majors included: ");
            // check for particular major or all
            if($row[1]){
              echo("$row[1]<br>"); 
            }
            else{
            ...
            // updates the DB with new values of the student limit etc
