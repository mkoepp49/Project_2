...
<html lang="en">
  <head>
    ...
    // Returns the element to manupulate
...
<?php
	//  gets the date and type selecte by admin
	$date = $_POST["date"];
	$type = $_POST["type"];
	...
	// Calls the display function for both, or just individual, or just group
	if($_POST["type"] == 'Both')
	{
...
?>
	<!--
	if back to home is pressed, goes back to advisor home
	If print, then calls onClick window.print() and prints window to pdf!
	-->
...
function displayGroup($id, $date)
{
	...
	// ******************************************************************
		...
		// get the advising appointments DB
       	$rs = $COMMON->executeQuery($sql, "Advising Appointments");
...
// Diplays individual appointments
function displayIndividual($id, $date)
{
	global $debug; global $COMMON;
		// get advisors appointments from DB
